...
describe("PostgreSQL browser runtime state store", () => {
  it("loads credential-free session and tab metadata", async () => {
    ...
    const database: RuntimeSqlExecutor = { query: async () => ({ rows: call++ === 0
      ...
      : [{ id: "tab-1", provider_search_id: null, session_id: "central-1", provider: "central-dispatch", source_filter_hash: "hash", status: "ready", created_at: "2026-09-24T12:00:00Z", last_scan_at: null }]
    }) };
